...
- [x] Redis sliding-window rate limiting in `rate_limit_middleware.py`
- [x] Alembic first migration: `alembic revision --autogenerate -m "initial"`
- [x] Redis session state management (replace in-memory dict)
- [x] APScheduler dropout re-engagement job (daily cron)
- [x] Pinecone `_embed()` — move SentenceTransformer to module-level singleton (perf)
- [x] llama.cpp token streaming for production inference
...
| 2026-03-08 | P2: Redis sliding-window rate limiting | feat(ratelimit): Redis sliding-window rate limiting middleware |
| 2026-03-08 | P2: Alembic initial migration (7 tables) | feat(alembic): initial autogenerated database migration |
| 2026-03-08 | P2: Redis session state management | feat(session): Redis-backed session state management |
| 2026-03-08 | P2: APScheduler dropout re-engagement cron | feat(scheduler): APScheduler daily dropout re-engagement cron |
| 2026-03-08 | P2: SentenceTransformer module-level singleton | feat(rag): move SentenceTransformer to module-level singleton |
| 2026-03-08 | P2: llama-cpp-python native token streaming | feat(inference): llama-cpp-python native token streaming for production |
